...
status1 = call('./apriori -f"," -s{0} -v"[Sup. %0S]" {1} apriori_temp1.txt'.format(minSup, filename), shell=True)
if status1!=0:
...
if minConf>0:
    ...
    status2 = call('./apriori -tr -f"," -n{0} -c{1} -s{2} -v"[Conf. %0C,Sup. %0S]" {3} apriori_temp2.txt'.format(maxRule, minConf, minSup, filename), shell=True)
    if status2!=0:
...
for i,line in enumerate(lines):
    ...
    sup[i] = re.findall(' \d*e.*]', line)[0][1:-1]
...
for i,line in enumerate(lines):
    ...
    conf[i] = re.findall(' \d*e.*,', line)[0][1:-1]
