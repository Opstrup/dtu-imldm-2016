...
for train_index, test_index in CV:
    ...
    ann_y_est = bestnet[k].sim(X_test)
    ann_y_est = (ann_y_est>.5).astype(int)
    ann_errors[k] = (ann_y_est!=y_test).sum().astype(float)/y_test.shape[0]
    k+=1

# Print the average classification error rate
print('Error rate: {0}%'.format(100*mean(ann_errors)))
print(ann_errors)


figure();
subplot(2,1,1); bar(range(0,K),ann_errors); title('CV errors');
subplot(2,1,2); plot(error_hist); title('Training error as function of BP iterations');
figure();
subplot(2,1,1); plot(ann_y_est); plot(y_test.A); title('Last CV-fold: est_y vs. test_y'); 
subplot(2,1,2); plot((ann_y_est-y_test).A); title('Last CV-fold: prediction error (est_y-test_y)'); show()
show()
